# Les Archives Miskatoniques — contexte projet

Catalogue communautaire de contenu fan-made francophone pour *Horreur à Arkham :
le Jeu de Cartes*. Voir [`README.md`](README.md) pour l'architecture générale.

## Structure des cycles fan-made

Le contenu est organisé en « cycles » (`data/catalogue.json` → `cycles`). Certains
cycles portent le nom « Les Dossiers d'Arkham N » : ce sont des assemblages de
scénarios fan-made indépendants (ou de scénarios/mini-campagnes hétérogènes)
regroupés pour reproduire la taille d'une campagne officielle ; les autres cycles
sont une unique campagne fan-made cohérente.

Les descriptions de `Cycle 1` et `Cycle 2` dans `catalogue.json` reflètent déjà ce
détail. Celles de `Cycle 3` à `Cycle 16` sont encore des placeholders génériques
(« Xème lot de contenu fan-made. Détails sur le Discord de la communauté. »).
Contenu réel de chaque cycle, pour référence future (ex. si on étoffe ces
descriptions) :

- **Cycle 1 — Les Dossiers d'Arkham 1** : huit scénarios fan-made indépendants —
  Face au Wendigo, Jouet d'une illusion, Le Festival, Le Palais du Rire, Le Pont
  des Soupirs, Le Wyrm de Pensher, Les Seuils de la Cryonce, Pulsions cynégétiques.
- **Cycle 2 — Les Dossiers d'Arkham 2** : même principe, huit scénarios
  indépendants — Désolation à bord du Constellation, La Symphonie d'Erich Zann,
  Les Chiens de Tindalos, Au bord du gouffre, Sang versé à Salem, La Couleur
  tombée du ciel, L'Événement de Svalbard, L'Hôtel du Grand Chêne.
- **Cycle 3 — Les Dossiers d'Arkham 3** : variation du principe, huit scénarios
  répartis en une campagne de 4 scénarios (Les Disparus de Killineq), une
  campagne de 3 scénarios (Au Cœur des ténèbres) et un scénario indépendant
  (La Maison vaudou).
- **Cycle 4** : campagne fan-made de 8 scénarios, *Matière noire*. Pas de nom
  « Dossiers » car composé d'une seule campagne.
- **Cycle 5** : campagne fan-made de 10 scénarios, *Dimensions étranges*.
- **Cycle 6** : campagne fan-made de 8 scénarios, *L'Appel du Porte-Peste* —
  vient tout juste d'être finalisée.
- **Cycle 7** : campagne fan-made de 8 scénarios, *Fondations cyclopéennes*.
- **Cycle 8 — Les Dossiers d'Arkham 4** : regroupement totalisant 8 scénarios —
  La Guerre des mondes (mini-campagne de 3 scénarios), La Légende du Cavalier
  sans tête (scénario indépendant), L'Étrange Cas du docteur Jekyll et de
  M. Hyde (scénario indépendant), La Sororité des secrets (scénario
  indépendant), De profundis (mini-campagne de 2 scénarios).
- **Cycle 9** : campagne fan-made de 8 scénarios, *Alice au pays des merveilles*.
- **Cycle 10** : campagne fan-made de 8 scénarios, *Les Ombres de Yog-Sothoth*.
- **Cycle 11 — Les Dossiers d'Arkham 5** : deux extensions fan-made d'une
  campagne officielle — Renouveau des Dévoreurs de rêves, Quête onirique
  étendue (extension des Dévoreurs de rêves). Nombre de scénarios difficile à
  dénombrer car elles s'appuient sur des scénarios existants.
- **Cycle 12 — Les Dossiers d'Arkham 6** : contient pour l'instant un scénario
  fan-made, *Noël à Arkham*. Sera complété par *Métro 2033* (en cours de
  relecture).
- **Cycle 13** : campagne fan-made de 8 scénarios, *La Cité de l'Invisible* —
  traduction en cours.
- **Cycle 14** : campagne fan-made de 8 scénarios, *Circus Ex Mortis*.
- **Cycle 15** : campagne fan-made de 9 scénarios, *L'Ombre des Grands Anciens*
  — actuellement en cours d'impression groupée.
- **Cycle 16** : campagne fan-made de 8 scénarios, *Age Unwound* — en cours de
  traduction.
